...
namespace Rivet {
  ...
  class CMS_Internal_FSQ_15_007 : public Analysis {
  public:

    /// Constructor
    CMS_Internal_FSQ_15_007() : Analysis("CMS_Internal_FSQ_15_007") {}
    ...
    void analyze(const Event& event) {
      ...
      if (fabs(dphi_leadjet) < PI/3.) {   // Transverse region
          nTowards_leadjet++;
          ptSumTowards_leadjet += pT;
        }

     if (fabs(dphi_leadjet) > 2.*PI/3.) {   // Transverse region
          nAway_leadjet++;
  ...
  DECLARE_RIVET_PLUGIN(CMS_Internal_FSQ_15_007);
}
