...
def getHistos(filelist, filenames):
    ...
    refhistos, mchistos = {}, {}
    for infile, inname in zip(filelist, filenames): ##pairs together each infile with corresponding inname
        ...
        for path, ao in analysisobjects.items():
            ...
            try:
                aop = rivet.AOPath(path) ##class containing a YODA path with all its components 
            ...
            if infile in mchistos:
                for xpath in sorted(mchistos[infile]):
                    ...
                    if defaultWeightName not in hmcs:
                        print ("WARNING: Weight \"%s\" not found in file!" % defaultWeightName)
                    ...
                    for bandType, bandContribs in poptions.items():
                        for curvegroup in bandContribs.split(' '):
                            ...
                            errors = None; PDFset = None; PDFvars = [ ]
                            for varname in sorted(hmcs.keys()):
                                ...
                                if isNominal or any([ re.search(pat, varname) for pat in curvegroup.split(',') ]):
                                    ...
                                    if isPDF:
                                        ...
                                        errors[0] = list(map(min, zip(errors[0], central_values))) ##returns min y value
                                        errors[1] = list(map(max, zip(errors[1], central_values))) ##returns max y value
                            ...
                            if BANDNAME in hmcs and errors != None:
                                ...
                                for i in range(hmcs[BANDNAME].numPoints()):
                                    ...
                                    contrib0 = errors[0][i] if isPDF else val - errors[0][i] ##add exp. and stat. errs in quadrature 
                                    contrib1 = errors[1][i] if isPDF else val - errors[1][i]
                                    err0 = math.sqrt(err0 ** 2 + contrib0 ** 2)
                                    err1 = math.sqrt(err1 ** 2 + contrib1 ** 2)
                                    hmcs[BANDNAME].point(i).setYErrs(err0, err1) ##if not exp. errs, returns y errors as: pos err = max y input, neg err = min y input
